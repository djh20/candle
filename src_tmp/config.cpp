...
void PersistentConfig::load(const char* id)
{
  this->id = id;

  log_i("Loading config (ID: %s)", id);
...
bool PersistentConfig::getStatus() 
{
  return status;
}

void PersistentConfig::setStatus(bool newStatus)
{
...
char* PersistentConfig::getBluetoothName() {
  return bluetoothName;
}

void PersistentConfig::setBluetoothName(const char* newName) {
  prefs->begin(id, RW_MODE);
